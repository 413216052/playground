...
react作为某种程度上的主流前端框架，它有着许许多多的优点和思想，这些使得前端er们构建项目变得得心应手。不过仔细想一想，在一个重型应用中，项目的代码不断变的复杂，变态的视图操作不断变多，然而以当下的数据驱动理念的编码风格，我们虽不用过多的关心视图层面的问题，却依然能够流畅的在浏览器中运行，这是不是也有点太过轻松了？

那么是什么给我们提供了这种流畅的撸码体验，在肆无忌惮的撸码后，React帮助我们实现了什么呢？

-------------------

React中，render函数是一个组件的输出钩子，从这里可以很直观的看到输出到页面的节点，然而真正执行的结果得到的却不是真正的DOM节点，其结果是相比DOM较为轻量的js对象，即Virtual DOM。
...
之所以这样做，是因为在js-eventloop中是会一直占用浏览器主线程的，这个不结束，渲染就不执行。
